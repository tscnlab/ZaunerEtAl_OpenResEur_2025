...
We will start off by importing the data and setting factor levels. Note that the rating "Very unlikely" was written as "Very Unlikely" in the REDCap survey. It will be changed to the lower case version for consistency with the other ratings.
...
data <- 
data |> mutate(wrist_timestamp = wrist_timestamp |> parse_date_time(orders = "%d%m%y%H%M"))
#Setting Units
...
gtsave(demographics_sample %>% 
         as_gt, "demographics_sample.docx", path = "output/04_unused/")
...
gtsave(demographics_overall %>% 
         as_gt, "demographics_overall.docx", path = "output/04_unused/")
...
gtsave(
  ...
  "table_1_demographics_combined.png", path = "output/02_tables/")

gtsave(
  demographics_combined %>% as_gt, 
  "table_1_demographics_combined.docx", path = "output/02_tables/")
...
gtsave(country_table %>% as_gt, "country_table.docx", 
       path = "output/03_supplemental/")
...
ggsave("output/04_unused/Full_plot_rating.jpeg", 
       Full_plot_rating, width = 14, height = 16, dpi = 600)
